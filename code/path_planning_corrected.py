...
class ImprovedAStar:
    ...
    def adaptive_step_size(self, pos):
        ...
        if density >= 2:
            return self.step_small
        else:
            return self.step_large
...
# ==================== BEZIER CURVE SMOOTHING (FIXED) ====================

class BezierSmoother:
    """Cubic Bezier curve path smoothing - CORRECTED VERSION"""
    
    @staticmethod
    def cubic_bezier(P0, P1, P2, P3, env, num_points=10):
        """
        Generate cubic Bezier curve with collision checking
        """
    ...
    def smooth_path(path, env):
        ...
        FIXED: Jump by 3 points instead of 1 to avoid creating too many waypoints
        """
        if len(path) < 4:
            return path
        ...
        i = 0
        while i <= len(path) - 4:  # Changed condition
            P0, P1, P2, P3 = path[i], path[i+1], path[i+2], path[i+3]
            
            # Try to generate Bezier curve
            curve_segment = BezierSmoother.cubic_bezier(P0, P1, P2, P3, env, num_points=10)
            
            if curve_segment is not None:
                # Use Bezier curve (skip first point to avoid duplication)
                smoothed.extend(curve_segment[1:])
                i += 3  # Jump 3 points forward
            else:
                # Curve collides, add next waypoint and move forward by 1
...
def create_experiment_environment():
    ...
    obstacles = [
        (2, 0, 1, 3), (5, 2, 1, 4), (8, 0, 1, 5),
        (11, 3, 1, 3), (14, 1, 1, 4), (3, 6, 2, 1),
        (7, 7, 3, 1), (12, 6, 2, 1), (2, 10, 1, 3),
        (6, 11, 1, 3), (10, 10, 1, 4), (14, 12, 1, 3),
        (4, 15, 2, 1), (9, 16, 2, 1), (13, 15, 2, 1),
    ]
...
def run_experiments():
    ...
    if path:
        ...
        results['trad'] = {
            'path': path, 'nodes': nodes, 'time': time_s,
            'length': length, 'turns': turns
        }
        
        print(f"      ✓ Success!")
        print(f"      → Length: {length:.2f} m, Turns: {turns}, Nodes: {nodes}, Time: {time_s*1000:.2f} ms")
    ...
    if 'imp' in results:
        ...
        smoothed = bezier.smooth_path(results['imp']['path'], env)
        ...
        if results['imp']['turns'] > 0:
            turn_red = (results['imp']['turns'] - turns) / results['imp']['turns'] * 100
            print(f"      → Turn reduction: {turn_red:.2f}%")
    
    # Hybrid
    print("\n[4/4] Running Hybrid Algorithm...")
    ...
    for key, name in [('trad', 'Traditional A*'), ('imp', 'Improved A*'), 
                       ('smooth', 'Improved A* + Bezier'), ('hybrid', 'Hybrid Algorithm')]:
        if key in results:
            r = results[key]
            time_str = f"{r['time']*1000:.2f}" if 'time' in r else '-'
            nodes_str = str(r['nodes']) if 'nodes' in r else '-'
            print(f"{name:<30} {time_str:<12} {r['turns']:<8} {r['length']:<12.2f} {nodes_str:<8}")
    
    if 'trad' in results and 'hybrid' in results:
        t, h = results['trad'], results['hybrid']
...
def visualize_results(results, env, start, goal):
    ...
    algorithms = [
        ('trad', 'Traditional A*'),
        ('imp', 'Improved A* (Adaptive)'),
        ('smooth', 'Improved A* + Bezier'),
    ...
    for idx, (key, title) in enumerate(algorithms):
        ...
        if key in results and 'path' in results[key]:
            ...
            color, width = colors.get(key, ('blue', 2))
            ax.plot(path_x, path_y, color=color, linewidth=width, 
                   label='Path', zorder=3, alpha=0.8)
            
            # Metrics box
            metrics = f"Length: {results[key].get('length', 0):.2f} m\n"
            metrics += f"Turns: {results[key].get('turns', 0)}\n"
            if 'time' in results[key]:
                metrics += f"Time: {results[key]['time']*1000:.2f} ms\n"
            if 'nodes' in results[key]:
                metrics += f"Nodes: {results[key]['nodes']}"
            
            ax.text(0.03, 0.97, metrics, transform=ax.transAxes,
    ...
    plt.tight_layout()
    plt.savefig('path_planning_corrected.png', dpi=300, bbox_inches='tight')
    print("\n✓ Visualization saved: 'path_planning_corrected.png'")
    plt.show()
